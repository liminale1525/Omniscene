...
assert.match(source, /createFeatureRuntime\(\{[\s\S]*imageDirect:[\s\S]*import\('\.\/qianmu-image-direct\.js\?v=1\.59\.65'\)[\s\S]*featureRuntime\.load\('imageDirect'\)/, 'the direct image runtime must enter the shared on-demand feature boundary');
assert.match(source, /optionalService:[\s\S]*import\('\.\/qianmu-service-capabilities\.js\?v=1\.59\.65'\)/, 'optional backend capability checks must stay outside the startup graph');
...
assert.match(source, /theaterCatalog:[\s\S]*Promise\.all\([\s\S]*import\('\.\/builtin-theaters\.js\?v=1\.59\.65'\)[\s\S]*import\('\.\/qianmu-theaters\.js\?v=1\.59\.65'\)/, 'both managed theater catalogs must share one on-demand feature boundary');
