...
assert.equal(manifest.version, '1.59.59');
...
assert.match(source, /const VERSION = '1\.59\.59';/);
...
assert.match(source, /qianmu-icon-renderer\.js\?v=1\.59\.59/, '稳定版必须按版本加载局部图标渲染器');
assert.match(source, /qianmu-storyboard\.js\?v=1\.59\.59/, '分镜数据契约必须随发布版本破除子模块缓存');
